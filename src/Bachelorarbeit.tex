...
%    \subsection{Start der Matrix Session}\label{sec:start-der-matrix-session}
    ...
    Der erste Schritt um eine Verschlüsselung der Nachrichten zu erreichen ist es beim Start der Matrix Session die End-to-End Verschlüsselung zu aktivieren.
    Dies geschieht durch die von der SDK bereitgestellte funktion \texttt{enableCrypto}.
    Hierdurch erstellt die SDK die in Abschnitt~\ref{subsec:verwendete-schlussel} aufgelisteten Schlüssel und synchronisiert diese mit dem Homeserver.
    Außerdem

    Um zu bestätigen das die Kommunikation zwischen
