...
\end{figure}
    ...
    Im Olm Algorithmus wird darauf verzichtet, sowohl eine Sending- als auch eine Receiving-Chain zu führen und durch eine einzelne Kette erzetzt.
    Ein Teilnehmer nutzt die geraden Schüssel der Kette und der ander Teilnehmer die ungeraden Schlüssel.
    ...
    \end{displaymath}
    Sollte der Chain-Key nicht passen werden ein neuer Root-Key und Chain-Key bestimmt durch hinzunahme eines generierten Rachet-Keys $T_i$.
    \begin{displaymath}
    ...
    Eine Megolm Session besitzt drei Teilen, einem Zähler $i$, einem Ed25519-Schlüsselpaar, welches zur Autentifizierung
    des Senders genutzt wird und dem Rachtet, der zur Verschlüsselung der Nachrichten verwendet wird.
    Der Rachet selbst besteht aus vier Teilen, die bei der Verkettung über verschiedene Hash-Funktionen fortgeführt werden.
    Damit ein anderer User die Nachrichten entschlüsseln kann muss er die Informationen der Session über eine sichere Peer-to-Peer-Verbindung an diesen übermitteln.
    Um vergangene Nachrichten zu entschlüsseln sollte der alteste, bekannte Ratchet Wert übergeben werden~\cite{megolm}.
    ...
    \begin{enumerate}
        ...
        \item Forward Secrecy ist nur teilwesie gegeben, da User den ältsten, bekannten Rachet Wert speichern, um alle Nachrichten einer Session zu entschlüsseln. Sollte ein Angreifer an diesen Schlüssel gelangen, kann er ebenfalls alle Nachrichten entschlüsseln.
    \end{enumerate}
...
target 'MyApp' do
  ...
  pod 'SwiftyJSON', '~> 2.3'
    ...
    Sie ermöglich eine einfache Kommunikation zwischen Client und Matrix Server.
...
%    \end{wrapfigure}
    ...
    Hat der User seine Zugangsdaten eingegeben und über den Login Button bestätigt, wird über den \texttt{MXRestClient}, welcher von der SDK (siehe Abschnitt~\ref{sec:matrix-sdk}) bereitgestellt wird, eine Login-Anfrage mit Usernamen und Passwort an den Server gesendet.
    ...
    Für den Fall, dass der User noch keinen Account besitzt, kann über den Button "Create Account" der \texttt{LoginView} zur \texttt{CreateAccountView} gelangen, welche in Abbildung~\ref{fig:createaccountview} dargestellt ist.
    ...
    Des Weiteren wird dem User ein Typing-Indikator gezeigt, welcher der User darüber informiert, ob der Gesprächspartner (oder in Gruppenräumen welche Gesprächspartner) gerade eine Nachricht schreiben.
