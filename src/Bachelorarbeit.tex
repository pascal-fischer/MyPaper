...
%    So sollte beispielsweise darauf geachtet werden, dass
    ...
    Die Arbeit
    Zu Beginn wird der theoretische Hintergrund über die in der App gen
    ...
    Des weiteren werden die in der End-to-End Verschlüsselung genutzten keys erläutert und genauer auf die verschiedenen Verschlüsselungsalgorithmen eingegangen.
...
{"menu": {
  ...
  "popup": {
    ...
    Dies geschieht durch verkettung einer Key Derivation Function (KDF)~\cite{perrin2016double}.
...
%    \subsection{Start der Matrix Session}\label{sec:start-der-matrix-session}
    ...
    Der erste Schritt um eine Verschlüsselung der Nachrichten zu erreichen ist es beim Start der Matrix Session End-to-End Verschlüsselung zu aktivieren.
    Dies geschieht durch die von der SDK bereitgestellte funktion \texttt{enableCrypto}.
    Hierdurch erstellt die SDK die in Abschnitt~\ref{subsec:verwendete-schlussel} aufgelisteten Keys und synchronisiert diese mit dem Homeserver.
    Außerdem

    Um zu bestätigen das die
...
registration_requires_token: true
    ...
    Dies würde die parallele Nutzung des gleichen Accounts auf mehreren Geräten ermöglichen.
